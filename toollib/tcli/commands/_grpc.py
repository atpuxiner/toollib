...
class Cmd(BaseCmd):
    ...
    def tpl2bash(self):
        ...
        with open(proto_path, "wb") as f1, open(server_path, "wb") as f2, open(client_path, "wb") as f3:
    ...
    def __replace(text_bytes: bytes, name: str):
        return (
            text_bytes.replace(b"hello", name.encode("utf8"))
            .replace(b"HELLO", name.upper().encode("utf8"))
            .replace(b"Hello", name.title().encode("utf8"))
        )
