...
class Cmd(BaseCmd):
    ...
    def tpl2bash(self):
        ...
        for item in opts.split(",") if opts else "":
            item = item.strip()
            if not item or item in ["h/help", "h", "help", "version"]:
                continue
            opts_error_msg = (
                f"ERROR: {item}: 格式错误（不能包含空格，短选项单字符，长选项多字符，后可接`:`表示需要值，如：s/src:）"
            )
            if "/" in item:
        ...
        with open(file, "w", encoding="utf8") as fp:
            fp.write(
                bash_tpl.BASH_TPL.replace("@SHORT_OPTS", short_opts)
                .replace("@LONG_OPTS", long_opts.rstrip(","))
                .replace("@DEFAULT_FALSE_OPTS", default_false_opts)
                .replace("@ALL_OPTS", all_opts)
                .replace("@ALL_FUNCS", all_funcs.rstrip())
                .replace("@ALL_CMDS", all_cmds.rstrip())
                .replace("@CMDS_STR", cmds.replace(",", "|"))
            )
